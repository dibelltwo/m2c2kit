...
import { getAssessmentConfigsForPackage, getSurveyConfigForPackage, } from "./package-protocol";
    ...
    if (event.type === "SESSION_START") {
        ...
        for (const activity of session.options.activities) {
            if (activity instanceof Survey) {
                activity.setParameters(buildEmaSurveyJson(getSurveyConfigForPackage(protocol, packageId), {
                    prompt_id,
            ...
            const game = activity;
            const assessmentConfig = getAssessmentConfigsForPackage(protocol, packageId).find((candidate) => candidate.activity_id === game.id);
        ...
        console.log(`[EMA] Starting session for prompt ${prompt_id}, package: ${packageId ?? "default"}, protocol: ${protocol?.study_id ?? "unknown"}`);
        session.start();
    }
});
