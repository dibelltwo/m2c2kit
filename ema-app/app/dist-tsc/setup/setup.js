...
function asEditableQuestion(item) {
  return {
    ...item,
    localId: makeLocalId(),
  };
}
function normalizeQuestionBank(protocol) {
...
function toPackageAssessment(assessment) {
  return {
...
function normalizePackages(protocol) {
...
function toEditableTimeBlock(block) {
  return { ...block, localId: makeLocalId() };
}
function normalizeScheduleRules(protocol, packageIdFallback) {
...
export function loadSavedProtocol() {
...
function buildDefaultAssessment(activityId) {
...
function buildInitialState(protocol) {
...
function toQuestionBankItems(items) {
  return items.map(({ localId: _localId, ...item }) => item);
}
function buildWarningMessage(state) {
...
function buildProtocolFromState(state) {
  const daysTotal =
    calculateInclusiveDayCount(state.start_date ?? "", state.end_date ?? "") ??
    state.days_total;
...
function addVersionBump(state) {
  state.version += 1;
}
export function mountSetupUI(root, initialProtocol, onSave) {
  ...
  `;
  const formEl = root.querySelector(".setup-form");
  const warningEl = root.querySelector("#setup-warning");
  const previewEl = root.querySelector("#setup-preview");
  const renderPreview = () => {
  ...
  const render = () => {
    formEl.innerHTML = `
      <div class="setup-section">
        ...
        <div class="setup-pill-row">
          ${ASSESSMENT_LIBRARY.map((assessment) => `<span class="setup-pill">${escapeHtml(assessment.label)}</span>`).join("")}
        </div>
      </div>
    ...
    `;
    const questionBankEl = formEl.querySelector("#setup-question-bank");
    const packagesEl = formEl.querySelector("#setup-packages");
    const rulesEl = formEl.querySelector("#setup-rules");
    ...
    formEl
      ...
      ?.addEventListener("input", (event) => {
        state.study_id = event.target.value.trim();
        renderPreview();
      });
    formEl
    ...
    formEl
      ...
      ?.addEventListener("input", (event) => {
        state.default_package_id = event.target.value;
        renderPreview();
      });
    [
      ...
      formEl.querySelector(selector)?.addEventListener("input", (event) => {
        state.surveyChrome[key] = event.target.value;
        renderPreview();
      });
    });
    formEl
      ...
      ?.addEventListener("input", (event) => {
        state.surveyChrome.instruction = event.target.value;
        renderPreview();
      });
    formEl
      ...
      ?.addEventListener("input", (event) => {
        ...
        if (daysTotal !== null) {
          ...
          const daysInput = formEl.querySelector("#setup-days-total");
    ...
    formEl
      ...
      ?.addEventListener("input", (event) => {
        state.days_total = Number(event.target.value);
        renderPreview();
      });
    questionBankEl.querySelectorAll("[data-question]").forEach((el) => {
    ...
    packagesEl.querySelectorAll("[data-package]").forEach((el) => {
      ...
      if (el.dataset.role === "assessment-enabled") {
        el.addEventListener("input", (event) => {
          ...
          const assessmentId = el.dataset.assessment;
          if (input.checked) {
            if (
    ...
    rulesEl.querySelectorAll("[data-rule]").forEach((el) => {
