...
function promptIdToInt(promptId: string): number {
  let hash = 0;
...
export class Scheduler {
  private prompts: ScheduledPrompt[] = [];
  readonly tracker = new ComplianceTracker();

  constructor(
    private protocol: StudyProtocol,
    private participantId: string,
  ) {}

  /** Generate schedule and register all local notifications. */
  async start(startDate: Date = new Date()) {
  ...
  private listenForTaps() {
    LocalNotifications.addListener(
      ...
      (action: ActionPerformed) => {
        const promptId = action.notification.extra?.prompt_id as
          | string
          | undefined;
        if (!promptId) return;
