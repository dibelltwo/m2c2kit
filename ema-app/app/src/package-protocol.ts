...
function toAssessmentConfig(pkg: StudyPackage): AssessmentConfig[] {
...
function buildSurveyFromQuestionBank(
...
export function getAvailablePackages(protocol: StudyProtocol): StudyPackage[] {
  ...
  if (protocol.default_package_id) {
    return (
      protocol.packages.find(
        (candidate) => candidate.package_id === protocol.default_package_id,
      ) ?? protocol.packages[0]
    );
  }
...
export function getAssessmentConfigsForPackage(
  protocol: StudyProtocol,
  packageId?: string,
): AssessmentConfig[] {
  const pkg =
...
export function getSurveyConfigForPackage(
  protocol: StudyProtocol,
  packageId?: string,
): EmaSurveyConfig {
  const pkg =
