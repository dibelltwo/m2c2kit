...
export class Scheduler {
    ...
    async start(startDate = new Date()) {
        this.prompts = generateSchedule(this.protocol, this.participantId, startDate);
        // Persist to tracker
    ...
    runExpiryCheck() {
        return checkExpiry(this.prompts, this.tracker, (prompt) => this.getExpiryMinutes(prompt));
    }
    getSchedule() {
        return [...this.prompts];
    }
    ...
    getExpiryMinutes(prompt) {
        if (!prompt.rule_id || !this.protocol.schedule_rules) {
            return this.protocol.schedule.expiry_minutes;
        }
        return (this.protocol.schedule_rules.find((rule) => rule.rule_id === prompt.rule_id)?.expiry_minutes ?? this.protocol.schedule.expiry_minutes);
    }
    async scheduleNotifications() {
        const now = Date.now();
        const future = this.prompts.filter((p) => new Date(p.scheduled_for).getTime() > now);
    ...
    listenForTaps() {
        LocalNotifications.addListener("localNotificationActionPerformed", (action) => {
            ...
            const sentAt = entry.sent_at
                ...
                : Date.now();
            const expiryMs = this.getExpiryMinutes(prompt) * 60000;
