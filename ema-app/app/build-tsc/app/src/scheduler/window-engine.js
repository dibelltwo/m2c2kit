 ...
 */
function randomizeWithinWindows(date, windows, totalPrompts, minGapMinutes) {
  ...
  const minGapMs = minGapMinutes * 60000;
  const results = [];
 ...
 */
function distributeEvenly(date, windows, totalPrompts) {
  const spans = windows.map((w) => ({
    start: parseHHMM(date, w.start).getTime(),
    end: parseHHMM(date, w.end).getTime(),
  }));
